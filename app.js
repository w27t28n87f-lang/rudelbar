...
let creatorState={produkt:"hoodie",farbe:"#111111",groesse:"M",view:"front",bg:"light",scale:1,x:0,y:0,rot:0,flipX:1,flipY:1};
...
function creatorUIRendern(){
  ...
  document.querySelectorAll("#creatorAnsichten [data-view]").forEach(b=>b.classList.toggle("aktiv",b.dataset.view===creatorState.view));
...
function creatorDesignBounds(){
  ...
  let base=creatorState.produkt==="cap"?170:creatorState.produkt==="bag"?280:creatorState.view==="left"||creatorState.view==="right"?180:280;
  ...
  let cx=450+creatorState.x, cy=(creatorState.produkt==="cap"?455:creatorState.produkt==="bag"?470:creatorState.view==="left"||creatorState.view==="right"?455:430)+creatorState.y;
  return {cx,cy,dw,dh};
}
...
function creatorProduktFotoZeichnen(ctx,w,h){
  ...
  ctx.save();
  // Heller Studio-Untergrund, damit schwarze Textilien klar erkennbar sind.
  const card=ctx.createLinearGradient(0,0,0,h);
  card.addColorStop(0,"#f7f7f7"); card.addColorStop(1,"#d9d9d9");
  ctx.fillStyle=card; ctx.beginPath(); ctx.roundRect(36,36,w-72,h-72,28); ctx.fill();
  ctx.shadowColor="rgba(0,0,0,.24)"; ctx.shadowBlur=22; ctx.shadowOffsetY=10;
  let sx=18,sy=18,sw=img.naturalWidth-36,sh=img.naturalHeight-36;
  if(creatorState.view==="left"){ sw*=.58; }
  if(creatorState.view==="right"){ sx+=sw*.42; sw*=.58; }
  const maxW=creatorState.view==="left"||creatorState.view==="right"?700:650;
  const maxH=creatorState.view==="left"||creatorState.view==="right"?700:650;
  const ratio=sw/sh; let dw=maxW,dh=dw/ratio; if(dh>maxH){dh=maxH;dw=dh*ratio;}
  const dx=(w-dw)/2,dy=(h-dh)/2+8;
  if(creatorState.view==="back"){ ctx.translate(w,0); ctx.scale(-1,1); ctx.drawImage(img,sx,sy,sw,sh,w-dx-dw,dy,dw,dh); }
...
document.querySelectorAll("#creatorAnsichten [data-view]").forEach(b=>b.onclick=()=>{creatorState.view=b.dataset.view;creatorUIRendern();creatorZeichnen();});
